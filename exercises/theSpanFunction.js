function span(arr, predicate) {
  ...
  for (var i = 0,n = arr.length; i<n; i++) {
  	current = arr[i];
  	if (breakpoint && predicate(current)) {
  		ok.push(current);
  	}else {
  		failed.push(current);
  		breakpoint = false;
...
The first array contains all the elements of the argument array up to 
the item that caused the first failure of the predicate.
